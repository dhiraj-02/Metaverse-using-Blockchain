...
var canvas = document.createElement("canvas");
    ...
    var texture = new THREE.CanvasTexture(canvas);
		var material = new THREE.MeshBasicMaterial( { map: texture } );
    const rec = "Meta"
    ctx.fillRect(0.2, 0.2, 6, 7);
    ctx.fillStyle = "green";
    ctx.fillRect(0, 0, 0, 0);
    ctx.fillStyle = "white";
    ctx.font = "17px Aerial";
    ctx.fillText(rec, 2.5, 20);
    var cube = new THREE.Mesh( new THREE.PlaneGeometry, material );
    const { forward,backward, left , right , jump , shift } = useInput();
    // const model = useGLTF("http://localhost:3000/new_final_avatar1.glb")
    const model = useGLTF("http://localhost:3000/office/sitting_boy_ac.glb")
    const {actions} = useAnimations(model.animations,model.scene);
    model.scene.add(cube);
    //model.scene.remove(cube);
    model.scene.position.set(70,1.2,-2.5);
    model.scene.scale.set(8,8,8);
    cube.scale.set(4.3,1.7,1.5);
    cube.position.set(4.6,2,1.6)
    cube.rotation.set(0,-1.57,0);
